...
Destroyer.prototype.fire = function () {
  ...
  for (i=1 ; i<this.range ; i++) {
    setTimeout(function () {
      place.x += direction.x;
      place.y += direction.y;
      if (['empty', 'deposit', 'boulder', 'pipe', 'salvageCamp', 'researchStation', 'destroyer'].includes(game.grid[this.pos.y + place.y][this.pos.x + place.x].type)) {
        if (
...
SalvageCamp.prototype.destroy = Deposit.prototype.destroy;
