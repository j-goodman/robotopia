...
var Deposit = function (x, y) {
  ...
  this.energy = Math.round(Math.random() * 225 + 25);
...
Deposit.prototype.onDestroy = function () {
  ...
  if (this.energy < this.totalEnergy) {
      game.tappedEnergy -= this.totalEnergy
  }
};
...
Contaminator.prototype.action = function () {
  ...
  if (this.cycle === this.interval * 1) {
    ...
    game.grid[this.pos.y + 2][this.pos.x - 1].poisoned = true;
    game.grid[this.pos.y + 1][this.pos.x - 2].poisoned = true;
  } else if (this.cycle === Math.floor(this.interval * 3.3)) {
    this.destroy();
  }
};
...
FusionReactor.prototype.action = function () {
    this.cycle += 1;
    if (this.cycle > this.cycleLength) {
        this.cycle = 0;
        game.energy += 1;
...
FusionReactor.prototype.onDestroy = FusionReactor.prototype.meltdown
