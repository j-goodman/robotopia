...
Destroyer.prototype.fire = function () {
  ...
  this.destroyableStructures = ['empty', 'deposit', 'boulder', 'pipe', 'extractor', 'researchStation', 'destroyer', 'rootflower', 'root', 'strangler', 'chemicalPlant', 'contaminator', 'shipwreck'];
  var direction;
  var i;
  var place;
  var plasma;
...
var Shipwreck = function (x, y) {
  this.type = 'shipwreck';
  this.contents = [new Item ('triggerpin')];
  this.pos = {
    x: x,
    y: y,
  };
...
Extractor.prototype.action = function () {
  ...
  if (this.cycle > this.cycleLength) {
    ...
    for (i=0 ; i<this.resources.length ; i++) {
      if (this.resources[i].type === 'deposit') {
...
Extractor.prototype.detectResources = function () {
  this.resources = [];
    if (['deposit', 'shipwreck'].includes(game.grid[this.pos.y + 1][this.pos.x].type)) {
    this.resources.push(game.grid[this.pos.y + 1][this.pos.x].tenant);
  } if (['deposit', 'shipwreck'].includes(game.grid[this.pos.y - 1][this.pos.x].type)) {
    this.resources.push(game.grid[this.pos.y - 1][this.pos.x].tenant);
  } if (['deposit', 'shipwreck'].includes(game.grid[this.pos.y][this.pos.x + 1].type)) {
...
ResearchStation.prototype.examineItems = function () {
  ...
  if (this.inventory.length === 0) {
    ...
    for (i=0 ; i<this.inventory.length ; i++) {
      ...
      element.innerText = game.itemData[this.inventory[i].tag].name;
