...
menus.displayMenu = function (object) {
  ...
  description.onclick = function () {
    var par;
    var description = game.tileData[object.type].description
    if (object.square.tenant.description) {
        description = object.square.tenant.description
    }
    ...
    content.innerHTML = '<li class="left-align" id="menu-description-paragraph">' + description + '</li>';
  ...
  if (object.square.tenant.itemCosts) {
    let itemTags = Object.keys(object.square.tenant.itemCosts)
    let itemCosts = object.square.tenant.itemCosts
    build.innerText = 'Manufacture'
    build.style.display = 'block'
    build.onclick = function () {
        object.square.tenant.manufactureItems()
    }.bind(this)
  }
  this.isOpen = true;
};
...
menus.appendAdditionalButtons = function (object) {
  ...
  switch (object.type) {
    ...
    case 'extractor':
      ...
      if (object.square.tenant.geoharvester) {
        element = document.createElement('LI');
        element.id = 'menu-remove-geoharvester';
        element.innerText = "Remove Geoharvester";
        element.onclick = object.square.tenant.removeGeoharvester.bind(object.square.tenant);
    ...
    case 'destroyer':
      ...
      content.appendChild(element);
      if (object.square.tenant.modifier === 'precisionDestroyer') {
        element = document.createElement('LI');
        element.id = 'menu-adjust-range';
        element.innerText = "Adjust Range [" + object.square.tenant.range + "]";
        element.onclick = object.square.tenant.adjustRange.bind(object.square.tenant, element);
        content.appendChild(element);
      }
      if (object.square.tenant.automatic) {
        element = document.createElement('LI');
        element.id = 'menu-remove-triggerpin';
        element.innerText = "Remove Triggerpin";
        element.onclick = object.square.tenant.removeTriggerpin.bind(object.square.tenant);
        content.appendChild(element);
      }
      element = document.createElement('LI');
    ...
    case 'researchStation':
      element = document.createElement('LI');
      element.id = 'menu-examine-items';
      element.innerText = "Examine Items";
      element.onclick = object.square.tenant.examineItems.bind(object.square.tenant);
      content.appendChild(element);
