  ...
  game.canvas.onclick = function () {
    ...
    if (game.cursor.mode === 'builder') {
      ...
      if (game.tiles[square.type].hasMenu && !menus.isOpen &&
          ((Math.abs(game.cursor.clickPos.x - game.cursor.x) + Math.abs(game.cursor.clickPos.y - game.cursor.y)) < 1)
      ) {
        this.menuHover = true;
        if (!this.unlocked) {
          menus.displayMenu({
...
addEventListener('mousemove', event => {
    if (game.cursor.clickDown) {
        let diff = {
            x: Math.floor(game.cursor.x - game.cursor.clickPos.x) * 2,
            y: Math.floor(game.cursor.y - game.cursor.clickPos.y) * 2,
        }
