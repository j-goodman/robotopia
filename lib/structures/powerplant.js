...
Powerplant.prototype.addToInventory = function (item) {
  ...
  if (!game.gotItemBefore) {
    ...
    text = `${itemData[item.tag].name} acquired.`;
    time = 3000;
  }
...
Powerplant.prototype.inventoryIncludes = function (tag) {
  var i;
  for (i=0 ; i<this.inventory.length ; i++) {
    if (this.inventory[i].tag === tag) {
      return true;
    }
  }
  return false;
...
Powerplant.prototype.removeFromInventory = function (itemTag) {
  var i;
  for (i=0 ; i<this.inventory.length ; i++) {
    if (this.inventory[i].tag === itemTag) {
...
Powerplant.prototype.action = function () {
  ...
  if (this.cycle > this.cycleLength) {
    ...
    if (game.energy + game.tappedEnergy < 11) {
      game.builder.builds.pipe.cost = 0
      game.builder.builds.extractor.cost = 0
    } else {
      game.builder.builds.pipe.cost = 1
      game.builder.builds.extractor.cost = 10
    }
    if (game.fusionReactors) {
        game.fusionReactors = game.fusionReactors < 0 ? 0 : game.fusionReactors
        game.energy += game.fusionReactors
    }
  }
