...
function drawGrid () {
  ...
  for (i=viewport.origin.y ; i<viewport.origin.y + viewport.height ; i++) {
    for (j=viewport.origin.x ; j<viewport.origin.x + viewport.width ; j++) {
      if (game.grid[i][j].poisoned) {
        game.ctx.drawImage(game.tiles.poisoned, (j - viewport.origin.x) * 20, (i - viewport.origin.y) * 20);
      } else {
  ...
  var displayPower = game.energy < 1000 ? game.energy.toString() : '999+'
