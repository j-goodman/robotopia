...
function drawGrid () {
  ...
  if (game.cursor && !game.cursor.clickDown) {
    game.viewport.pixelOffset.x -= game.viewport.pixelOffset.x > 0 ? 1 : 0
    game.viewport.pixelOffset.x += game.viewport.pixelOffset.x < 0 ? 1 : 0
    game.viewport.pixelOffset.y -= game.viewport.pixelOffset.y > 0 ? 1 : 0
    game.viewport.pixelOffset.y += game.viewport.pixelOffset.y < 0 ? 1 : 0
  }

  for (i=viewport.origin.y - 1 ; i<viewport.origin.y + viewport.height + 1 ; i++) {
    for (j=viewport.origin.x - 1 ; j<viewport.origin.x + viewport.width + 1 ; j++) {
