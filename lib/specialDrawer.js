...
function drawSuperExtractor (i, j, viewport) {
  ...
  if (game.grid[i][j].tenant.geoharvester) {
    game.ctx.drawImage(itemData.geoharvester.image, (j - viewport.origin.x) * 20, (i - viewport.origin.y) * 20);
  }
}

function drawGeoharvesterExtractor (i, j, viewport) {
  game.ctx.drawImage(game.tiles.extractor, (j - viewport.origin.x) * 20, (i - viewport.origin.y) * 20);
  game.ctx.drawImage(itemData.geoharvester.image, (j - viewport.origin.x) * 20, (i - viewport.origin.y) * 20);
}
