      ...
      if (i === 0 || i === game.settings.grid.height - 1 ||
        j === 0 || j === game.settings.grid.width - 1) {
          ...
          if (Math.floor(Math.random() * game.settings.depositRarity)) {
            ...
            if (!Math.floor(Math.random() * game.settings.shipwreckRarity)) {
              square = new Square ({type: 'shipwreck'}, j, i);
              object = new game.structures.lookup.shipwreck (j, i);
            } else {
...
mapbuilder.addFeatures = function () {
    ...
    mapbuilder.clearCircle({x: x, y: y}, 6, ['rootflower', 'deposit', 'powerplant', 'shipwreck'], 0.5)
}
...
mapbuilder.addPlayer = function () {
  ...
  mapbuilder.clearCircle({x: spawn.x, y: spawn.y}, 5, ['powerplant'], .2)
