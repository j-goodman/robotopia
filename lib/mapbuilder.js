...
mapbuilder.addPlayer = function () {
  ...
  let safetyOffset = Math.round(Math.random()) ? 3 : -3;
  let safetyDeposit = new Deposit (game.settings.spawn.y, game.settings.spawn.x + safetyOffset);
  game.grid[game.settings.spawn.x + safetyOffset][game.settings.spawn.y].type = 'deposit';
  game.grid[game.settings.spawn.x + safetyOffset][game.settings.spawn.y].tenant = safetyDeposit;
